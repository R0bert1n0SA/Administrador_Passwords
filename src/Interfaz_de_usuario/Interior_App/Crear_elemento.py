...
from PyQt5.QtWidgets import QAbstractItemView,QMessageBox,QListWidget,QGroupBox,QCheckBox,QAction,QComboBox,QMenuBar,QSpacerItem,QSizePolicy,QListWidgetItem ,QMenu,QApplication, QMainWindow, QLabel, QWidget, QVBoxLayout, QLineEdit, QPushButton, QFrame,QHBoxLayout
from PyQt5.QtCore import Qt,QSize,pyqtSignal,QFile,QTextStream
from PyQt5.QtGui import QFont,QIcon,QPixmap
from PyQt5.QtCore import pyqtSignal
from Funciones_Generales import *

class Crear_Pestania(QWidget):
    ...
    #Armando los contenedores
        self.CargaContenedor(Nombre,nombre,Inombre,70)
        self.CargaContenedor(Usuario,usuario,Iusuario,70)
        self.Cargar_Input_password(IPassword_Cont,Ipassword,boton,boton2,boton3)
        self.CargaContenedor(Password,password,IPassword_Cont,70)
        self.CargaContenedorPrincipal(Datos,Nombre,Usuario,Password)
    #------------------------------------------------------------------
        boton2.clicked.connect( lambda :toggle_visibility(Ipassword,boton2))
        ''' 
        self.generador.clicked.connect(self.enviar)
        '''
        boton.clicked.connect(lambda:self.Comprometida(Ipassword.text()))
    ...
    def edit(self,fol,nue):
        ind=self.opciones.findText(fol)
        if ind != -1:
            self.opciones.setItemText((ind),nue)
